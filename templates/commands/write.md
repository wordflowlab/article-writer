---
description: 多模式写作(教练/快速/混合/框架约束) - 必须选择模式
...
**必须执行**：
- ✅ 必须展示完整的模式选择界面（包含 1/2/3/4 四个选项）
- ✅ 必须等待用户明确输入 1、2、3 或 4
- ✅ 只有在用户输入数字后才能继续后续流程
...
2. **展示模式选择界面**：
   ...
   请选择写作模式：
     ...
     4. 📋 框架约束模式 - 你给框架，AI 填内容  [如果推荐，显示 ⭐ 推荐] ⭐ PRD-11 新增
        适合：项目报告、开题报告、标书、技术方案、专利申请
        工作原理：用户提供固定框架 → AI 按框架逐章生成 → 一致性检查
        时间投入：1.5-3 小时 | AI 检测率：20-35% | 质量：⭐⭐⭐⭐
        特点：框架严格固定，支持模板复用，自动检查一致性
...
3. **等待用户选择** ⚠️⚠️⚠️ 强制停止
   ...
   ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
   ⛔ AI 必须在此停止，等待用户输入 1、2、3 或 4
   ⛔ 禁止自动选择！禁止跳过！禁止假设用户选择！
   ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
```
✅ 确认选择：【1/2/3/4】【🎓教练模式 / ⚡快速模式 / 🔄混合模式 / 📋框架约束模式】
```
...
- **选择 2** → 跳转到"模式 2: 快速模式"
- **选择 3** → 跳转到"模式 3: 混合模式"
- **选择 4** → 跳转到 `/outline` 命令（框架约束模式）⭐ PRD-11 新增
...
**有固定框架要求时**：
- 领导给的提纲、客户指定的要点、行业标准模板
- 项目立项报告、开题报告、标书、专利申请、技术方案
- 框架不能随意修改，需要严格按结构填充内容

**推荐**：选择模式 4（框架约束模式），将自动跳转到 `/outline` 命令

---

# 模式 4: 📋 框架约束模式 (Outline-Constrained Mode) ⭐ PRD-11 新增

> **你的角色**: 内容填充助手
> **边界等级**: `constrained` - 框架固定，内容灵活
> **核心原则**: 严格遵循用户提供的框架结构，逐章生成高质量内容
...
**选择模式 4 后，AI 将自动引导你执行 `/outline` 命令**
...
你选择了框架约束模式，这个模式需要你提供固定的框架/提纲。

请执行 /outline 命令开始：
- /outline               → 直接粘贴提纲
- /outline --list        → 查看可用模板
- /outline --template XX → 使用预置模板

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```

详细的框架约束模式工作流程请参考 `/outline` 命令文档。
...
使用 `/outline --list` 查看所有可用模板。
也可以自定义模板，详见 `/outline` 命令文档。

---

**核心原则**：根据文章类型和时间预算，选择最合适的模式。质量和效率的平衡点由你自己决定。如果有固定框架要求，优先选择模式 4（框架约束模式）。
