...
**时间预算检查**：
- 可用时间：【如用户提供则计算，否则跳过】
- 时间充裕度：【✅ 充裕 / ⚠️ 一般 / ❌ 紧张】（仅在用户明确提供时间约束时评估）
...
⚠️ 时间成本：预估 6-9 小时 + 3天测试，时间紧张
⚠️ 质量目标：预期 AI 味 30-35%，略高于 < 30% 目标
✅ 专业门槛：产品评测，用户有使用经验

风险点：
1. 测试时间可能不足，建议预留更多准备时间
...
## 第三步：框架来源识别 ⭐ PRD-11 新增

> **核心原则**: 根据用户是否已有固定框架，决定后续流程
...
b) 📋 我已有框架（固定不可改）⭐ PRD-11
   - 领导/客户给了提纲
   - 必须严格按框架写
   - 适合：项目报告、开题报告、标书、技术方案
...
建议流程：
/specify (当前) → /outline (输入提纲) → /review → /publish

⚠️ 您可以先完成 brief 收集，也可以直接执行 /outline 开始框架约束模式。

继续收集需求信息...
```
→ 在 brief 中标记"框架来源: 用户提供"
→ 跳转到"第四步：处理用户需求"
...
建议流程：
/specify (当前) → /outline (输入要点) → /review → /publish

继续收集需求信息...
```
→ 在 brief 中标记"框架来源: 要点约束"
→ 跳转到"第四步：处理用户需求"
...
**如果是"用户提供框架"或"要点约束"**：
- 推荐流程：/specify → /outline → /review → /publish
- 推荐命令：`/outline` 框架约束模式
